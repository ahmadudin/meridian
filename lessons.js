...
export function evolveThresholds(perfData, config) {
  ...
  // ── 1. maxBinStep ──────────────────────────────────────────────
  // If losers tend to cluster at higher bin steps (proxy for high-volatility pools)
  // → tighten the bin-step ceiling. If winners span high bin steps safely → loosen a bit.
  // NOTE: pool.volatility is the per-pool metric; maxBinStep is the screening threshold
  // that acts as a proxy for volatility tolerance in the config schema.
  {
    ...
    if (loserVols.length >= 2) {
      ...
      if (loserP25 < current) {
        ...
        const newVal  = clamp(nudge(current, target, MAX_CHANGE_PER_STEP), 80, 500);
        const rounded = Number(newVal.toFixed(0));
        if (rounded < current) {
          changes.maxBinStep = rounded;
          rationale.maxBinStep = `Losers clustered at volatility ~${loserP25.toFixed(1)} — tightened maxBinStep from ${current} → ${rounded}`;
        }
      }
      ...
      if (winnerP75 > current * 1.1) {
        ...
        if (rounded > current) {
          changes.maxBinStep = rounded;
          rationale.maxBinStep = `All ${winners.length} positions profitable — loosened maxBinStep from ${current} → ${rounded}`;
        }
      }
    }
  }
